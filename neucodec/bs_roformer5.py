...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, x):
        # Accumulate in fp32 for numerical stability when x is fp16/bf16.
        norm_x = torch.mean(x.float() ** 2, dim=-1, keepdim=True)
        output = x * torch.rsqrt(norm_x + self.eps).to(x.dtype) * self.weight
        return output
